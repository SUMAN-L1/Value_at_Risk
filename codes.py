...
# ===================== Helper: pure-numpy OLS =====================
def numpy_ols(X_raw, y):
    """
    Fit OLS via numpy.linalg.lstsq.
    X_raw : 2-D array WITHOUT intercept column
    y     : 1-D array
    Returns: params (intercept first), residuals, resid_std
    """
    n = len(y)
    ...
    y_hat   = X @ params
    resids  = y - y_hat
    resid_std = resids.std(ddof=len(params))
    return params, resids, resid_std
...
uploaded_file = st.file_uploader(
    "Choose your file (CSV, Excel)", type=["csv", "xls", "xlsx"],
    help="File should contain dates and market prices"
)

if not PLOTLY_AVAILABLE and interactive_plots:
    st.markdown(
        '<div class="warning-box"><strong>⚠️ Note:</strong> Interactive charts not available. '
        'Install plotly for better visualizations.</div>',
        unsafe_allow_html=True,
    )
...
5. Always have emergency funds to cultivate one crop at least
...
""")
            ...
            if interactive_plots and PLOTLY_AVAILABLE:
                ...
                fig.add_vline(x=mc_cvar,   line=dict(color="purple", dash="dash"),
                              annotation_text=f"CVaR {to_pct(mc_cvar):.2f}%",
                ...
                fig.update_layout(title=f"Risk Distribution & Methods - {col}",
                                  xaxis_title="Log Returns", yaxis_title="Frequency / scaled density",
                                  bargap=0.1, height=450, showlegend=True)
        ...
        # ===================== Advanced Analysis =====================
        # Detect price column (Modal or first numeric) and arrivals column
        price_col    = "Modal"    if "Modal"    in df.columns else None
        arrivals_col = "Arrivals" if "Arrivals" in df.columns else None

        if price_col and arrivals_col:
            st.markdown('<h2 class="sub-header">🔍 Advanced Risk Testing</h2>', unsafe_allow_html=True)

            # --- Build clean dataset using only numpy/pandas basics (no statsmodels) ---
            ds = df[[price_col, arrivals_col]].copy()

            # Log returns of price
            ds["Log_Returns"] = np.log(
                ds[price_col].replace(0, np.nan) /
                ds[price_col].replace(0, np.nan).shift(1)
            )

            # Log arrivals – replace zeros then forward/back fill with pandas native
            log_arr = np.log(ds[arrivals_col].replace(0, np.nan))
            log_arr = log_arr.ffill().bfill()          # pure pandas, no fillna(method=...)
            ds["Log_Arrivals"] = log_arr

            ds = ds.dropna(subset=["Log_Returns", "Log_Arrivals"])
            ...
            if len(ds) > rw:
                ...
                for i in range(rw, len(ds)):
                    train = ds.iloc[i - rw: i]
                    test  = ds.iloc[i]

                    X_tr = train["Log_Arrivals"].values
                    y_tr = train["Log_Returns"].values

                    # Pure-numpy OLS — no statsmodels, no fillna
                    params, resids, resid_std = numpy_ols(X_tr.reshape(-1, 1), y_tr)
                    intercept, slope = params[0], params[1]

                    arr_mu  = X_tr.mean()
                    arr_std = X_tr.std()

                    sim_arr  = np.random.normal(arr_mu, arr_std, int(num_simulations))
                    sim_mu   = intercept + slope * sim_arr
                    sim_ret  = np.random.normal(sim_mu, resid_std, int(num_simulations))
                ...
                def safe_log(v):
                    return np.log(v) if v > 0 else -1e10
                ...
                # Backtest plot
                last_col = analysis_cols[-1] if analysis_cols else price_col
                if interactive_plots and PLOTLY_AVAILABLE:
                    st.markdown("### 📊 Interactive CVaR / Monte Carlo Backtest (with Kupiec Test)")
                    fig = go.Figure()
                    fig.add_trace(go.Scatter(
                        x=bt_df.index, y=(np.exp(bt_df["Actual_Return"]) - 1) * 100,
                        mode="lines+markers", name="Actual Weekly Returns (%)",
                        hovertemplate="Date: %{x}<br>Actual Return: %{y:.2f}%<extra></extra>",
                    ))
                    fig.add_trace(go.Scatter(
                        x=bt_df.index, y=(np.exp(bt_df["MC_VaR"]) - 1) * 100,
                        mode="lines", name="Monte Carlo VaR (%)",
                        line=dict(color="red", dash="dash"),
                        hovertemplate="Date: %{x}<br>VaR: %{y:.2f}%<extra></extra>",
                    ))
                    fig.add_trace(go.Scatter(
                        x=bt_df.index, y=(np.exp(bt_df["MC_CVaR"]) - 1) * 100,
                        mode="lines", name="Conditional VaR (%)",
                        line=dict(color="purple", dash="dot"),
                        hovertemplate="Date: %{x}<br>CVaR: %{y:.2f}%<extra></extra>",
                    ))
                    fig.update_layout(
                        title=f"Backtest: Actual vs MC VaR/CVaR ({last_col}) — Kupiec LR={LR_uc:.2f} p={kupiec_pvalue:.3f}",
                    ...
                    if not breaches.empty:
                        ax.scatter(breaches.index, (np.exp(breaches["Actual_Return"]) - 1) * 100, label="VaR Breach", color="red", zorder=5)
                    ax.set_title(f"Backtest ({last_col}) — Kupiec LR={LR_uc:.2f} p={kupiec_pvalue:.3f}")
                ...
                with c1: st.metric("Observed Breach Rate", f"{x / n * 100:.2f}%")
                with c2: st.metric("Expected Breach Rate", f"{p * 100:.2f}%")
                with c3: st.metric("Kupiec p-value", f"{kupiec_pvalue:.3f}")

                if kupiec_pvalue < 0.05:
                    st.markdown('<div class="warning-box"><strong>⚠️ Kupiec Test:</strong> Reject the null — actual breaches exceed expected. Model may need recalibration.</div>', unsafe_allow_html=True)
                else:
                    st.markdown('<div class="success-box"><strong>✅ Kupiec Test:</strong> Accept the null — actual breaches match expected. Model is well-calibrated.</div>', unsafe_allow_html=True)

                breach_rate   = bt_df["Breach_CVaR"].mean() * 100
                expected_rate = (1 - confidence_level) * 100

                c1, c2, c3 = st.columns(3)
                with c1: st.metric("Model Calibration (approx)", f"{100 - abs(breach_rate - expected_rate):.1f}%")
                with c2: st.metric("Actual Breach Rate (CVaR)",  f"{breach_rate:.1f}%")
                with c3: st.metric("Expected Breach Rate (CVaR)", f"{expected_rate:.1f}%")
            ...
            if run_forecast:
                ...
                train_latest = ds.iloc[-rw:]
                X_lt = train_latest["Log_Arrivals"].values
                y_lt = train_latest["Log_Returns"].values

                params_lt, _, resid_std_lt = numpy_ols(X_lt.reshape(-1, 1), y_lt)
                intercept_lt, slope_lt = params_lt[0], params_lt[1]

                mu_arr    = X_lt.mean()
                sigma_arr = X_lt.std()

                forecast_cvars, forecast_vars = [], []
                for h in range(1, int(forecast_horizon) + 1):
                    np.random.seed(42 + h)
                    sim_arr_f   = np.random.normal(mu_arr, sigma_arr, int(num_simulations))
                    sim_mu_f    = intercept_lt + slope_lt * sim_arr_f
                    sim_ret_f   = np.random.normal(sim_mu_f, resid_std_lt, int(num_simulations))
                    var_f       = np.percentile(sim_ret_f, (1 - confidence_level) * 100)
                    cvar_f      = sim_ret_f[sim_ret_f <= var_f].mean()
                    forecast_cvars.append((h, (np.exp(cvar_f) - 1) * 100))
                    forecast_vars.append( (h, (np.exp(var_f)  - 1) * 100))
                ...
                first_sl   = fc_df["Predicted_CVaR (%)"].iloc[:min(5, len(fc_df))]
                last_sl    = fc_df["Predicted_CVaR (%)"].iloc[max(0, len(fc_df) - 5):]
                risk_trend = "INCREASING" if last_sl.mean() < first_sl.mean() else "STABLE"
                ...
                if interactive_plots and PLOTLY_AVAILABLE:
                    fig = go.Figure()
                    fig.add_trace(go.Scatter(x=forecast_df.index, y=forecast_df["Predicted_CVaR (%)"],
                                             mode="lines+markers", name="CVaR Forecast",
                                             line=dict(color="red", width=3)))
                    fig.add_trace(go.Scatter(x=forecast_df.index, y=forecast_df["Predicted_VaR (%)"],
                                             mode="lines+markers", name="VaR Forecast",
                                             line=dict(color="orange", width=2)))
                    fig.add_vline(x=worst_idx, line=dict(color="red", dash="dash", width=2),
                                  annotation_text=f"Highest Risk Week {worst_idx}", annotation_position="top")
                    fig.update_layout(title=f"Risk Forecast for Next {forecast_horizon} Weeks",
                                      xaxis_title="Weeks Ahead", yaxis_title="Potential Weekly Loss (%)", height=450)
                    st.plotly_chart(fig, use_container_width=True)
                else:
                    fig, ax = plt.subplots(figsize=(12, 5))
                ...
                if worst_val < -10:
                    st.markdown(
                        ...
                        f'<h4>🚨 HIGH ALERT</h4><p><strong>High risk detected around week {worst_idx}!</strong></p></div>',
                        unsafe_allow_html=True)
                elif worst_val < -5:
                    st.markdown(
                        f'<div style="background-color:#fff3cd;padding:15px;border-radius:10px;border-left:5px solid #ffc107;">'
                        f'<h4>⚠️ CAUTION</h4><p><strong>Moderate risk around week {worst_idx}</strong></p>'
                        f'<ul><li>Monitor market closely around week {worst_idx}</li>'
                        f'<li>Keep 7–10 days buffer stock</li>'
                        f'<li>Have backup buyers ready</li>'
                        f'<li>Save 10% of revenue for emergencies</li></ul></div>',
                        unsafe_allow_html=True)
                else:
                    st.markdown(
                        f'<div style="background-color:#d4edda;padding:15px;border-radius:10px;border-left:5px solid #28a745;">'
                        f'<h4>✅ NORMAL</h4><p><strong>Low risk expected — normal operations recommended</strong></p>'
                        f'<ul><li>Standard farming and selling practices should work well</li>'
                        f'<li>Good time to plan expansion or investments</li>'
                        f'<li>Maintain normal inventory levels</li></ul></div>',
                        unsafe_allow_html=True)
    ...
    if st.button("📊 Load Sample Data", type="primary"):
        ...
        modal_prices    = np.maximum(base_price + seasonal_factor + trend + noise, 20)
        arrivals_seasonal = -200 * np.sin(2 * np.pi * np.arange(len(dates)) / 52 + np.pi / 4)
        arrivals        = np.maximum(1000 + arrivals_seasonal + np.random.normal(0, 100, len(dates)), 100)
        ...
        st.download_button("📥 Download Sample Data", sample_df.to_csv(index=False),
                           "sample_market_data.csv", "text/csv")
...
<div style="text-align:center;padding:20px;background-color:#f8f9fa;border-radius:10px;margin-top:30px;">
    ...
    <p style="font-size:12px;color:#999;margin-top:15px;">
        Version 2.2 | Enhanced for Professional Use | Updated {pd.Timestamp.now().strftime("%B %Y")}
    </p>
</div>
""",
    unsafe_allow_html=True,
