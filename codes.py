...
.section-title {
    ...
    font-weight: 700;
    padding: 0.5rem 0;
    border-bottom: 3px solid #2e6b46;
...
[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #0d2b1f 0%, #1a4731 100%);
}
[data-testid="stSidebar"] * { color: #d4edd9 !important; }
[data-testid="stSidebar"] .stSlider > div > div > div { background: #2e6b46 !important; }

/* DataFrame */
.dataframe { font-size: 0.85rem; }

/* Expander */
.streamlit-expanderHeader { font-size: 0.95rem !important; font-weight: 600 !important; }
...
def compute_var_metrics(series, conf, n_sims, mu_override=None, sigma_override=None):
    ...
    np.random.seed(42)
    sims  = np.random.normal(mu_override or mu,
                              sigma_override or sigma, int(n_sims))
...
def plot_var_distribution(log_ret, m, title, use_plotly):
    """Single combined distribution plot with all four VaR/CVaR lines."""
    hist_var, param_var, mc_var, mc_cvar = (
        m["hist_var"], m["param_var"], m["mc_var"], m["mc_cvar"]
    )
    if use_plotly and PLOTLY:
        ...
        fig.add_trace(go.Scatter(x=xs, y=pdf, mode="lines",
                                  name="Parametric (Normal)", line=dict(color="#2196F3",width=2)))
        for val, col, name in [
            (hist_var,  "#222",    f"Historical VaR  {to_pct(hist_var):.2f}%"),
        ...
        xs = np.linspace(log_ret.min(), log_ret.max(), 300)
        scale = len(log_ret)*(log_ret.max()-log_ret.min())/50
        ax.plot(xs, norm.pdf(xs, m["mu"], m["sigma"])*scale, "--", color="#2196F3", label="Parametric Normal")
        ax.axvline(hist_var,  color="#222",    ls="--", lw=2, label=f"Historical VaR {to_pct(hist_var):.2f}%")
        ax.axvline(param_var, color="#2196F3", ls="--", lw=2, label=f"Parametric VaR {to_pct(param_var):.2f}%")
        ax.axvline(mc_var,    color="#e74c3c", ls="--", lw=2, label=f"MC VaR {to_pct(mc_var):.2f}%")
        ax.axvline(mc_cvar,   color="#8e44ad", ls="--", lw=2, label=f"CVaR {to_pct(mc_cvar):.2f}%")
        ax.set_title(title); ax.set_xlabel("Log Returns"); ax.legend(fontsize=8)
...
def policy_brief(market, conf, m, rl):
    ...
    farmer_actions = {
        ...
        "LOW": [
            ...
            "Document current price levels to use as baseline for future comparisons",
        ],
    }
    policy_actions = [
...
| Method | Log Return VaR | % Weekly Loss |
|---|---|---|
...
with col_up1:
    uploaded = st.file_uploader(
        "Upload CSV, XLS, or XLSX file (must contain a Date column and at least one numeric price column)",
        type=["csv","xls","xlsx"]
    )
with col_up2:
    st.markdown("""
...
with st.expander("📊 No data? Generate & download sample data"):
    ...
    if st.button("Generate Sample Data", type="primary"):
        ...
        modal = (50 + 12*np.sin(2*np.pi*t/52) + 0.08*t
                  + np.random.normal(0,6,len(dates)))
        modal = np.maximum(modal, 15)
        arr   = (1000 - 220*np.sin(2*np.pi*t/52 + np.pi/4)
                  + np.random.normal(0,120,len(dates)))
        arr   = np.maximum(arr, 80)
...
if not numeric_cols:
    st.error("❌ No numeric columns found. Check your file.")
    st.stop()
...
with c2:
    price_col = st.selectbox("Price column to analyse", numeric_cols,
                              index=numeric_cols.index("Modal") if "Modal" in numeric_cols else 0)
with c3:
...
if len(summary_rows) > 1:
    st.markdown('<div class="section-title">🔀 Cross-Column Risk Comparison</div>', unsafe_allow_html=True)
    sum_df = pd.DataFrame(summary_rows)
    st.dataframe(sum_df, use_container_width=True, hide_index=True)

    if use_plotly and PLOTLY:
        fig = go.Figure()
        for method, col_key in [("Hist VaR (%)","#4a9e6e"),("MC VaR (%)","#e74c3c"),("CVaR (%)","#8e44ad")]:
            fig.add_trace(go.Bar(name=method, x=sum_df["Market / Column"],
                                  y=sum_df[method].abs(), marker_color=col_key))
...
if not has_adv:
    ...
    To enable backtesting, Kupiec test, OLS regression and future forecasting,
    ...
    # ── OLS Regression ──
    st.markdown('<div class="section-title">📉 OLS Regression: Returns ~ Log Arrivals</div>', unsafe_allow_html=True)
    ols = run_ols(ds)
    corr = ds[["Log_Returns","Log_Arrivals"]].corr().iloc[0,1]

    r1,r2,r3,r4 = st.columns(4)
    r1.metric("Intercept (α)",  f"{ols.params[0]:.6f}")
    r2.metric("Coefficient (β)",f"{ols.params[1]:.6f}")
    ...
    if show_backtest and len(ds) > window:
        st.markdown('<div class="section-title">🔬 Rolling Backtest — Monte Carlo VaR & CVaR</div>', unsafe_allow_html=True)

        np.random.seed(42)
        bt_rows = []
        prog = st.progress(0, "Running rolling backtest…")
        total_steps = len(ds) - int(window)
        for i in range(int(window), len(ds)):
            ...
            sim_m = mdl.params[0] + mdl.params[1]*sim_a
            ...
            bt_rows.append({
                ...
                "MC_VaR":        vl,
                "MC_CVaR":       cv,
                "Breach_VaR":    actual < vl,
                "Breach_CVaR":   actual < cv,
            })
            prog.progress((i - int(window)+1) / total_steps)
        prog.empty()
        bt = pd.DataFrame(bt_rows).set_index("Date")

        # Kupiec test
        n, x = len(bt), bt["Breach_VaR"].sum()
        ...
        if use_plotly and PLOTLY:
            ...
            fig.add_trace(go.Scatter(x=bt.index, y=act_pct, mode="lines",
                                      name="Actual Returns (%)", line=dict(color="#2e6b46",width=1.5)))
            fig.add_trace(go.Scatter(x=bt.index, y=var_pct, mode="lines",
                                      name="MC VaR (%)", line=dict(color="#e74c3c",dash="dash")))
            fig.add_trace(go.Scatter(x=bt.index, y=cvar_pct, mode="lines",
                                      name="CVaR (%)", line=dict(color="#8e44ad",dash="dot")))
            ...
            ax.plot(bt.index,(np.exp(bt["Actual_Return"])-1)*100,label="Actual",color="#2e6b46",lw=1.5)
            ax.plot(bt.index,(np.exp(bt["MC_VaR"])-1)*100,  label="MC VaR",  color="#e74c3c",ls="--")
            ax.plot(bt.index,(np.exp(bt["MC_CVaR"])-1)*100, label="CVaR",    color="#8e44ad",ls="-.")
            breaches = bt[bt["Breach_VaR"]]
            if not breaches.empty:
                ax.scatter(breaches.index,(np.exp(breaches["Actual_Return"])-1)*100,
        ...
        if kp < 0.05:
            ...
            ⚠️ <strong>Kupiec Test: REJECT H₀</strong> — Observed breach rate significantly differs from expected.
            Model may be mis-calibrated. Consider adjusting window or distribution assumptions.
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown("""<div class="ok-box">
            ✅ <strong>Kupiec Test: ACCEPT H₀</strong> — Observed breach rate is statistically consistent
            with the model's expected failure rate. Model is well-calibrated.
            </div>""", unsafe_allow_html=True)

        if abs(breach_rate_cvar - expected_rate) < 2:
            st.markdown("""<div class="ok-box">
            ✅ <strong>CVaR Calibration:</strong> Breach rate closely matches nominal rate.
            </div>""", unsafe_allow_html=True)
        elif breach_rate_cvar > expected_rate + 2:
            st.markdown("""<div class="warn-box">
            🔴 <strong>CVaR is underestimating tail risk</strong> — actual tail losses exceed model predictions.
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown("""<div class="ok-box">
            🟢 <strong>CVaR appears conservative</strong> — model is over-predicting losses (safe side).
            </div>""", unsafe_allow_html=True)
    ...
    # ─────────────────────────────────────────────
    if show_forecast:
        st.markdown('<div class="section-title">🔮 Future Risk Forecast</div>', unsafe_allow_html=True)
        ...
        for h in range(1, int(horizon)+1):
            ...
            sim_m  = mdl_latest.params[0] + mdl_latest.params[1]*sim_a
        ...
        if use_plotly and PLOTLY:
            ...
            fig.update_layout(title=f"Risk Forecast — Next {horizon} Weeks",
                               xaxis_title="Weeks Ahead", yaxis_title="Potential Weekly Loss (%)",
                               height=400, plot_bgcolor="#f9fffe", paper_bgcolor="#f9fffe")
            st.plotly_chart(fig, use_container_width=True)
        else:
            fig,ax = plt.subplots(figsize=(12,5))
            ax.plot(fc_df.index,fc_df["CVaR (%)"],marker="o",lw=2,label="CVaR",color="#e74c3c")
            ax.plot(fc_df.index,fc_df["VaR (%)"], marker="s",lw=2,label="VaR", color="#f39c12")
            ax.axvline(worst_w,color="#e74c3c",ls="--",label=f"Worst Week {worst_w}")
            ax.fill_between(fc_df.index,fc_df["CVaR (%)"],-50,alpha=0.08,color="#e74c3c")
        ...
        with st.expander("📋 Full Forecast Table"):
            disp = fc_df[["VaR (%)","CVaR (%)"]].copy()
            disp["VaR (%)"]  = disp["VaR (%)"].round(3).astype(str) + "%"
            disp["CVaR (%)"] = disp["CVaR (%)"].round(3).astype(str) + "%"
        ...
        if worst_v < -10:
            ...
            Expected worst-case weekly loss exceeds <strong>10%</strong>. Immediate risk mitigation required.<br>
            • Reduce perishable inventory now · Activate forward contracts · Notify FPO immediately
            </div>""", unsafe_allow_html=True)
        elif worst_v < -5:
            st.markdown(f"""<div class="risk-mod">
            ⚠️ <strong>CAUTION — Week {worst_w}</strong><br>
            Moderate tail risk (5–10% loss). Maintain buffer stock, monitor arrivals, keep backup buyers ready.
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown(f"""<div class="risk-low">
            ✅ <strong>NORMAL CONDITIONS</strong><br>
            Risk levels are low over the forecast horizon. Good time for expansion and investment planning.
            </div>""", unsafe_allow_html=True)
...
# ─────────────────────────────────────────────
if summary_rows:
    st.markdown('<div class="section-title">📋 Overall Risk Summary</div>', unsafe_allow_html=True)
    st.dataframe(pd.DataFrame(summary_rows), use_container_width=True, hide_index=True)
    csv_out = pd.DataFrame(summary_rows).to_csv(index=False).encode()
    st.download_button("⬇️ Download Risk Summary CSV", csv_out, "risk_summary.csv","text/csv")
...
<div class="footer">
    ...
    <div style="font-size:0.9rem; color:#82c99a; margin-bottom:1rem;">
        This application applies financial risk econometrics (VaR, CVaR, Monte Carlo simulation,
        OLS regression, Kupiec backtesting) to agricultural commodity markets
        to help farmers and policymakers make data-driven decisions.
    </div>
